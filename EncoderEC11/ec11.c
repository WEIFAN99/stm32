...
void Encoder_EC11_Init(unsigned char Set_EC11_TYPE)
{
    ...
    EC11_A_Last = HAL_GPIO_ReadPin(EC11_A_GPIO_Port,EC11_A_Pin);   
    EC11_B_Last = HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin);
...
//  static char EC11_B_Last = 0;
    ...
    {                       //======================================================//
        if(HAL_GPIO_ReadPin(EC11_A_GPIO_Port,EC11_A_Pin) != EC11_A_Last)   //以A为时钟，B为数据。正转时AB反相，反转时AB同相
        {
            if(HAL_GPIO_ReadPin(EC11_A_GPIO_Port,EC11_A_Pin) == 0)
            {
                if(HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin) ==1)      //只需要采集A的上升沿或下降沿的任意一个状态，若A下降沿时B为1，正转                    
                    ScanResult = 1;     //正转

                else                    //反转
                    ScanResult = -1;
            }
            EC11_A_Last = HAL_GPIO_ReadPin(EC11_A_GPIO_Port,EC11_A_Pin);   //更新编码器上一个状态暂存变量
            EC11_B_Last = HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin);   //更新编码器上一个状态暂存变量
        }
    }                       //======================================================//
    else                    //================两定位对应一脉冲的EC11================//
    {                       //======================================================//
        if(HAL_GPIO_ReadPin(EC11_A_GPIO_Port,EC11_A_Pin) !=EC11_A_Last)        //当A发生跳变时采集B当前的状态，并将B与上一次的状态进行对比。
        {                                   //若A 0->1 时，B 1->0 正转；若A 1->0 时，B 0->1 正转；
                                            //若A 0->1 时，B 0->1 反转；若A 1->0 时，B 1->0 反转
            if(HAL_GPIO_ReadPin(EC11_A_GPIO_Port,EC11_A_Pin) == 1)     //EC11_A和上一次状态相比，为上升沿
            {
                if((EC11_B_Last == 1)&&(HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin) == 0))   //EC11_B和上一次状态相比，为下降沿
                    ScanResult = 1;                         //正转

                if((EC11_B_Last == 0)&&(HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin) == 1))   //EC11_B和上一次状态相比，为上升沿               
                    ScanResult = -1;                        //反转

                //>>>>>>>>>>>>>>>>下面为正转一次再反转或反转一次再正转处理<<<<<<<<<<<<<<<<//
                if((EC11_B_Last == HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin))&&(HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin) == 0))  //A上升沿时，采集的B不变且为0
                    ScanResult = 1;                                 //正转

                if((EC11_B_Last == HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin))&&(HAL_GPIO_ReadPin(EC11_B_GPIO_Port,EC11_B_Pin) == 1))  //A上升沿时，采集的B不变且为1
                    ScanResult = -1;                                //反转
            }
